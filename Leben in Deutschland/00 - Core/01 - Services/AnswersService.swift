...
class AnswersService: ObservableObject {
    ...
    private func loadAnswers() {
        if let data = UserDefaults.standard.data(forKey: UserDefaultsKeys.learningAnswers),
    ...
    private func persistAnswers() {
        if let encoded = try? JSONEncoder().encode(learningAnswers) {
            UserDefaults.standard.set(encoded, forKey: UserDefaultsKeys.learningAnswers)
        }
    }
}
